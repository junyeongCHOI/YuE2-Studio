...
처음 한 번은 AI 모델 파일(약 8GB)을 내려받아 Mac 에 맞게 바꾸느라 오래 걸립니다.
바꾸기가 끝나면 필요 없어진 원본은 알아서 지워요. 다음부터는 금방 켜집니다.
...
- 화면이 너무 어둡거나 밝으면 오른쪽 위 **테마** 에서 **라이트** 나 **다크** 를 고르세요.
- 마우스 없이 키보드 **Tab** 키로도 모든 버튼과 **?** 설명을 쓸 수 있어요.
- AI 모델은 노래를 만드는 동안에만 메모리를 써요. 다 만들고 나면 메모리를 돌려줘요.
- 원본 모델을 남겨 두고 싶으면 `./start --keep-originals` 로 켜세요.
